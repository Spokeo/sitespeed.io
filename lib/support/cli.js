...
module.exports.parseCommandLine = function parseCommandLine() {
  ...
  .option('webpagetest.host', {
    ...
    */
    ...
    .option('urlFile', {
      describe: 'Path to newline-deliminated file of URLs',
      alias: 'f'
    })
    .option('baseUrl', {
      describe: 'Prepend to all URL(s)'
    })
    .help('h')
    .alias('help', 'h')
    .config('config')
  ...
  return {
    urls: cliUtil.getURLs(argv._, explicitOptions.baseUrl, explicitOptions.urlFile),
    urlsMetaData: cliUtil.getAliases(argv._, explicitOptions.baseUrl, explicitOptions.urlFile),
    options: argv,
